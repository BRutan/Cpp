...
namespace CAD
{
	...
	// Constructors/Destructor:
	////////////////////////////
	Circle::Circle() : centerPoint(), m_radius(0), Shape()										/* Default constructor. Call centerPoint's default constructor and set radius to 0. */
	{
		
	}
	Circle::Circle(double x_in, double y_in) : centerPoint(x_in, y_in), m_radius(0), Shape()	/* Overloaded constructor. Set radius to 0 since only information known is the center point's coordinates. */
	{
		
	}
	Circle::Circle(const Circle &circle_in) : centerPoint(circle_in.centerPoint), m_radius(circle_in.m_radius), Shape(circle_in)			/* Copy constructor. */
	{
	...
	// Accessors:
	////////////////////////////
	Point Circle::CenterPoint() const					/* Return the center point object. */
	{
		return centerPoint;
	}
	double Circle::Radius() const						/* Return the value of the circle's radius. */
	{
		return m_radius;
	}
	////////////////////////////
	// Mutators:			  
	////////////////////////////
	void Circle::CenterPoint(const Point &point_in)		/* Set the state of the center point to passed point's state. */
	{
		centerPoint = point_in;
	}
	void Circle::Radius(const double radius_in)			/* Return m_radius. */
	{
		m_radius = radius_in;
	}
	////////////////////////////
	// Misc Methods:
	////////////////////////////
	double Circle::Area() const							/* Return the value of the circle's area. */
	{
		return M_PI * m_radius * m_radius;
	}
	double Circle::Circumference() const				/* Return the value of the circle's circumference. */
	{
		return 2 * M_PI * m_radius;
	}
	double Circle::Diameter() const						/* Return the value of the circle's diameter. */
	{
		return m_radius * 2;
	}
	void Circle::Draw() const							/* Draw the Circle object. */
	{
		std::cout << "Circle has been drawn. " << std::endl;
	}
	std::string Circle::ToString() const				/* Return the string description of the circle ("Circle(X(), Y()) Radius m_radius") */
	{
		std::stringstream ss;
	...
	/////////////////////////////////////////////////////////////////////
	// Member operators:
	Circle& Circle::operator=(const Circle& circle_in)						/* Assignment operator. */
	{
	...
	// Global operators:
	std::ostream& operator<<(std::ostream &os, const Circle &circle_in)		/* Overloaded ostream operator. */
	{
