...
namespace Options
{
	...
	{
		if (Type()) // Return price of Put option if option is Call using Put-Call Parity.
		{
			return Price() + Param("k") * exp(-Param("r") * Param("t")) - Param("s");
		}
		else		// Return price of corresponding Call option if option is Put.
		{
			return Price() - Param("k") * exp(-Param("r") * Param("t")) + Param("s");
		}
	}
	...
	double EuropeanOption::Gamma() const				/* Change in price with respect to s (2nd order). */
	{
		...
		return boost::math::cdf(normDist, D1()) / (Param("s") * Param("sigma") * sqrt(Param("t")));
	}
	double EuropeanOption::Vega() const					/* Change in price with respect to sigma (1st order). */
	{
		// Following Put-Call Parity, we see that Vega is the same for both Puts and Calls (dP/dsigma = dC/dsigma):
		return Param("k") * exp(-Param("r") * Param("t")) * boost::math::cdf(normDist, D1()) * sqrt(Param("t"));
	}
	double EuropeanOption::Theta() const				/* Change in price with respect to t (1st order). */
	{
		double outVal = 0;
		if (Type())
		{
			outVal -= Param("s") * boost::math::cdf(normDist, D1()) * Param("sigma") * exp((Param("b") - Param("r")) * Param("t"));
			outVal /= 2 * sqrt(Param("t"));
			outVal -= (Param("b") - Param("r")) * Param("s") * exp((Param("b") - Param("r")) * Param("t")) * boost::math::cdf(normDist, D1());
			outVal -= Param("r") * Param("k") * exp(-Param("r") * Param("t")) * boost::math::cdf(normDist, D2());
			return outVal;
		}
		else
		{
			outVal -= (Param("s") * boost::math::cdf(normDist, D1()) * Param("sigma")) / (2 * sqrt(Param("t")));
			outVal += Param("r") * Param("k") * exp(-Param("r") * Param("t")) * boost::math::cdf(normDist, -D2());
			return outVal;
		}
	}
