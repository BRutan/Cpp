...
-Member Functions:
	...
	*Option(const Option&): Copy constructor.
	// Accessors:
	*bool Type() const: Return type of option (true means Call, false means Put).
	*double Param(const string& key) const: Return passed parameter's value.
	// Mutators:
	*void TypeToggle(): Switch option to alternate type.
	*void Param(const string &key, double val): Set parameter to new passed value.
	// Misc Methods:
	*double FiniteDiffApprox(string, double, unsigned, double): Calculate Nth order approximation (n <= 66) for option Greek.
...
namespace Options
{
	...
	{ 
		...
		{
			throw Exceptions::OptionExcept("Option::FiniteDiff() Error: Parameter value must be > 0.");
		}
